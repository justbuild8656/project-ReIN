...
public class CraftManager : MonoBehaviour
{
    ...
    private Dictionary<ArtifactMaterialID, ArtifactMaterial> MaterialInventory;
    ...
    {
        reqCondition = 0;
        AddDictionary(ArtifactMaterialID.Gold, ArtifactMaterialGrade.Common, 5, MaterialInventory);
        AddDictionary(ArtifactMaterialID.Diamond, ArtifactMaterialGrade.UltraRare, 5, MaterialInventory);
        AddDictionary(ArtifactMaterialID.Ruby, ArtifactMaterialGrade.Rare, 5, MaterialInventory);
        AddDictionary(ArtifactMaterialID.MagicBook01, ArtifactMaterialGrade.SuperRare,5, MaterialInventory);
        for (int i = 0; i < recipe.reqMaterials.Length; i++)
        {
            RefreshSlot(i, recipe.reqMaterials[i].materialID.ToString(), recipe.reqMaterials[i].count, MaterialInventory[recipe.reqMaterials[i].materialID].count);
        }
    }
    ...
    private void Initialize()
    {
        reqCondition = 0;
        //인스턴스화
        MaterialInventory = new Dictionary<ArtifactMaterialID, ArtifactMaterial>();
        Slots = new List<GameObject>();

        //딕셔너리속 데이터들 null => 0으로 변경
        AddDictionary(ArtifactMaterialID.Gold,ArtifactMaterialGrade.Common, 0, MaterialInventory);
        AddDictionary(ArtifactMaterialID.Diamond, ArtifactMaterialGrade.UltraRare, 0, MaterialInventory);
        AddDictionary(ArtifactMaterialID.Ruby, ArtifactMaterialGrade.Rare, 0, MaterialInventory);
        AddDictionary(ArtifactMaterialID.MagicBook01, ArtifactMaterialGrade.SuperRare, 0, MaterialInventory);
        //테스트
        ...
        for (int i = 0; i < recipe.reqMaterials.Length; i++)
        {
            AddSlot(recipe.reqMaterials[i].materialID.ToString(), recipe.reqMaterials[i].count, MaterialInventory[recipe.reqMaterials[i].materialID].count);
        }
    ...
    private void CraftArtifact()
    {
        if(reqCondition == recipe.reqMaterials.Length)
        {
            ...
            for (int i = 0; i < recipe.reqMaterials.Length;i++)
            {
                Debug.Log("등급 :" + MaterialInventory[recipe.reqMaterials[i].materialID].grade.ToString());
                RemoveDictionary(recipe.reqMaterials[i].materialID, recipe.reqMaterials[i].count, MaterialInventory);
                RefreshSlot(i,recipe.reqMaterials[i].materialID.ToString(), recipe.reqMaterials[i].count, MaterialInventory[recipe.reqMaterials[i].materialID].count);
            }
    ...
    private void AddDictionary(ArtifactMaterialID itemID,ArtifactMaterialGrade grade ,int count, Dictionary<ArtifactMaterialID, ArtifactMaterial> inventory)
    {
    ...
    private void RemoveDictionary(ArtifactMaterialID itemID,int count, Dictionary<ArtifactMaterialID, ArtifactMaterial> inventory)
    {
        if (inventory.ContainsKey(itemID))
        {
            inventory[itemID].count -= count;
