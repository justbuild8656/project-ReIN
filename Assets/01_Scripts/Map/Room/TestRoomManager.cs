...
public class TestRoomManager : MonoBehaviour
{
  ...
  public void Start(){
    if(mapGenerator != null){
      ...
      if(tree != null){if (tree.root != null) LoadRoom(tree.root); else Debug.Log("루트 노드가 없음");}
  ...
  public void LoadRoom(RoomNode roomnode){
    if(room == null){Debug.Log("방 프리팹이 없음"); return;}
    ...
    if(roomLoadUI == null){Debug.Log("roomLoadUI가 없음"); return;}
  ...
  // 방 이벤트 설정 매서드 (현재는 임시로 텍스트만 표시)
  public void RoomEvent(GameObject currentRoom, RoomNode roomNode){
    TextMeshProUGUI text = currentRoom.GetComponentInChildren<Canvas>().GetComponentInChildren<TextMeshProUGUI>();

    text.text = $"Room ID: {roomNode.roomID} \n Room Type: {roomNode.roomType}";

    // 클리어 방 이라면 클리어 표시
    if(roomNode.roomType == RoomType.End) text.text += "\n\n\n\n\n\n\n Clear!";
  }
}
